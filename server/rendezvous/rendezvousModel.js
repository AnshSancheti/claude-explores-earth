...
const LOW_INFORMATION_URBAN_WORDS = new Set([
  ...
  'am', 'architectural', 'between', 'beyond', 'clear', 'cloudy', 'converging', 'corridor', 'corridors', 'dense', 'depth', 'detail', 'distinct', 'east', 'eastern', 'elements', 'empty', 'expansive', 'extend', 'extending', 'extends', 'facade', 'facades', 'flanked', 'form', 'formed', 'forming', 'forms', 'horizon', 'i', 'image', 'including', 'intersections', 'into', 'is', 'lane', 'lanes', 'large', 'lead', 'leading', 'ledge', 'left', 'lengthy', 'like', 'lined', 'lintel', 'local', 'long', 'm', 'minimal', 'multi',
...
function localObservationConcepts(value) {
  ...
  if (
    /\b(?:construction|scaffold)\w*\b/i.test(description) ||
    /\b(?:cone|barrier)s?\b[^.!;]{0,50}\b(?:safety|sidewalk|street|work)\w*\b/i
      .test(description) ||
    /\b(?:safety|sidewalk|street|work)\w*\b[^.!;]{0,50}\b(?:cone|barrier)s?\b/i
      .test(description)
  ) {
...
Return only JSON:
{
  ...
  "readableText": true | false
    ...
    if (
      contributionKind === 'local_observation' &&
      !localObservationMatchesBlindRead(contributionSummary, blindRead)
    ) {
      const competingSubject = cleanString(
        blindRead.primarySubject || blindRead.likelyMessage,
        300
