...
test('model decision adds own local corridor text when convergence policy overrides an editable sheet', async () => {
  const client = {
    chat: {
      completions: {
        async create() {
          return {
            choices: [{ message: { content: JSON.stringify({
              selectedIndex: 1,
              reasoning: 'I draw the movement line and keep going.',
              padOperations: [
                { type: 'line', from: { x: 0.1, y: 0.8 }, to: { x: 0.6, y: 0.25 } }
              ],
              passPad: false
            }) } }]
          };
  ...
  const result = await service.decide({
    ...
    options: [
      ...
      { panoId: 'street-east', heading: 135, label: 'W 17th St' }
    ],
...
test('model decision does not duplicate own local text when current own ink already has it', async () => {
      ...
      completions: {
        async create() {
          return {
            choices: [{ message: { content: JSON.stringify({
              selectedIndex: 1,
              reasoning: 'I follow the visible corridor.',
              padOperations: [],
              passPad: false
            }) } }]
          };
  ...
  const result = await service.decide({
    ...
    partnerName: 'Ada',
    options: [
      { panoId: 'south-option', heading: 208, label: '7th Ave' },
      { panoId: 'north-option', heading: 29, label: '7th Ave' }
    ],
    screenshots: [Buffer.from('south'), Buffer.from('north')],
    scratchpadBuffer: Buffer.from('pad'),
    canEditPad: true,
    padStatus: 'in your hands',
    ownPadText: ['W12TH ST'],
